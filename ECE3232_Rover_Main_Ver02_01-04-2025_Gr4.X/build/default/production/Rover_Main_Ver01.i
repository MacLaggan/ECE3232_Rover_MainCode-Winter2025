...
# 2 "<built-in>" 2
# 1 "Rover_Main_Ver01.c" 2
# 16 "Rover_Main_Ver01.c"
...
void HAL_Write_Init_Nibble(uint8_t nibble){
 ...
 byte = (byte | 0 | LCD._backlight);
 HAL_I2C_Master_Transmit(LCD.i2c_addr, &byte, 1);
 HAL_Pulse_Enable(byte);
}

void HAL_Write_Command(uint8_t cmd){
 _write_byte((cmd & 0xF0) | 0);
 _write_byte(((cmd<<4) & 0xF0) | 0);
 if (cmd <= 3){
...
void HAL_Write_Data(uint8_t data){
 _write_byte((data & 0xF0)| 1);
 _write_byte(((data << 4) & 0xF0) | 1);
}


void HAL_Clear(){
 HAL_Write_Command(0x01);
 _delay((unsigned long)((3)*(32000000/4000.0)));
}

void HAL_Show_Cursor(){
 HAL_Write_Command(0x08 | 0x04 | 0x02);
}

void HAL_Hide_Cursor(){
 HAL_Write_Command(0x08 | 0x04);
}

void HAL_Blink_Cursor_On(){
 HAL_Write_Command(0x08 | 0x04 | 0x02 | 0x01);
}
...
void initinit(){
 ...
 HAL_Write_Init_Nibble(0x30);
 _delay((unsigned long)((5)*(32000000/4000.0)));
 HAL_Write_Init_Nibble(0x30);
 _delay((unsigned long)((1)*(32000000/4000.0)));
 HAL_Write_Init_Nibble(0x30);
 _delay((unsigned long)((1)*(32000000/4000.0)));
 HAL_Write_Init_Nibble(0x20);
 _delay((unsigned long)((1)*(32000000/4000.0)));
 init_LCD();
}
# 48 "Rover_Main_Ver01.c" 2
# 1 "./TI_Handshake.h" 1
# 36 "./TI_Handshake.h"
uint16_t message = 0;
...
uint16_t TI_Recieve(){
    ...
    while((stopBit == 1)){
        ...
        if((PORTBbits.RB0 == 0) & (PORTBbits.RB1 == 0)){
            stopBit = 0;
            while((PORTBbits.RB0 & PORTBbits.RB1) == 0){
            }
            LATBbits.LATB0 = 0;
            LATBbits.LATB1 = 0;
...
void vectorCalculations(){
    int magnitude = sqrtf(right_Joystick_X * right_Joystick_X + right_Joystick_Y * right_Joystick_Y)+ 20;
    HAL_LED_OFF(1);
    HAL_LED_OFF(2);
    HAL_LED_OFF(3);
    HAL_LED_OFF(4);
    ...
    else if(right_Joystick_X < 0){
        angle = atanf(((float)right_Joystick_Y / (float)right_Joystick_X));
        if(angle > 0){
           HAL_LED_ON(3);
           if(angle < 3.14159265358979323846/4){


                speed = ((angle - 3.14159265358979323846/4)*-1)*127 + 20;
                MotorSettings(1,speed, 2, magnitude);
            }
            else{
                speed = (angle - 3.14159265358979323846/4)*127+ 20;
...
# 588 "Rover_Main_Ver01.c"
}


void IRdetection(){
}
# 608 "Rover_Main_Ver01.c"
void initialize_all(){
    LCD._backlight = 0x8;
